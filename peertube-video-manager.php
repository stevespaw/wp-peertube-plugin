...
class PeerTube_Video_Manager {
	...
	public function auto_display_video( $content ) {
		...
		if ( isset( $_GET['pt_video'] ) && ! empty( $_GET['pt_video'] ) ) {
			...
			$channel = isset( $_GET['pt_channel'] ) ? sanitize_text_field( $_GET['pt_channel'] ) : '';
			$video_shortcode = '[pt-video number="' . esc_attr( $video_number ) . '"' . ( ! empty( $channel ) ? ' channel="' . esc_attr( $channel ) . '"' : '' ) . ']';
				...
				function modifySearchForms() {
					try {
						...
						for (var i = 0; i < searchForms.length; i++) {
							...
							// Skip PeerTube-only search forms
							if (form.classList.contains('pt-peertube-search-form')) {
								continue;
							}
