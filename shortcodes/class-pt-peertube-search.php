...
class PT_PeerTube_Search {
	...
	public static function render_form( $atts ) {
		$atts = shortcode_atts( array(
			'placeholder' => __( 'Search in the media library', 'peertube-video-manager' ),
			'action'      => '',
		...
		if ( file_exists( $template_path ) ) {
			// Add special class to prevent WordPress search integration
			$form_class = 'pt-search-form pt-peertube-search-form';
			// We'll modify the template output
			ob_start();
			include $template_path;
			$form_html = ob_get_clean();
			// Replace form class
			$form_html = str_replace( 'class="pt-search-form"', 'class="' . esc_attr( $form_class ) . '"', $form_html );
			echo $form_html;
		} else {
	...
	public static function render_results( $atts ) {
		...
		if ( empty( $query ) && ! isset( $_GET['pt_search'] ) ) {
			return '<p class="pt-no-results">' . esc_html__( 'Please enter a search term.', 'peertube-video-manager' ) . '</p>';
		}
		...
		// Get default channels from settings for filtering
		$default_channels = get_option( 'pt_vm_default_channels', '' );
		$channel_handles = null;
		if ( ! empty( $default_channels ) ) {
			$channel_handles = preg_split( '/[\r\n,]+/', $default_channels );
			$channel_handles = array_map( 'trim', $channel_handles );
			$channel_handles = array_filter( $channel_handles );
			if ( empty( $channel_handles ) ) {
				$channel_handles = null;
			}
		}
		...
		if ( empty( $result['data'] ) ) {
			return '<p class="pt-no-results">' . sprintf(
				...
				esc_html__( 'No results found for "%s".', 'peertube-video-manager' ),
				esc_html( $query )
			) . '</p>';
		}
		...
		echo '<p>' . sprintf(
			...
			esc_html__( '%1$d results for "%2$s"', 'peertube-video-manager' ),
		...
		if ( $total > $per_page ) {
			...
			$links       = paginate_links( array(
				...
				'total'     => $total_pages,
				'type'      => 'array',
				'prev_text' => __( '« Back', 'peertube-video-manager' ),
				'next_text' => __( 'Next &raquo;', 'peertube-video-manager' ),
			) );
