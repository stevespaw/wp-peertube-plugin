<?php
/**
 * Channel videos shortcode
 *
 * @package PeerTube_Video_Manager
 * @since 1.0.0
 */

// Exit if accessed directly
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

/**
 * PT_Channel_Videos shortcode class
 *
 * @package PeerTube_Video_Manager
 * @since 1.0.0
 */
class PT_Channel_Videos {

	/**
	 * Register shortcode
	 *
	 * @since 1.0.0
	 */
	public static function register() {
		add_shortcode( 'pt-channel-videos', array( __CLASS__, 'render' ) );
	}

	/**
	 * Render shortcode
	 *
	 * @param array $atts Shortcode attributes.
	 * @return string Rendered output.
	 * @since 1.0.0
	 */
	public static function render( $atts ) {
		$atts = shortcode_atts( array(
			'channel' => '',
			'count'   => 6,
			'columns' => 'auto',
		), $atts, 'pt-channel-videos' );

		$channel = sanitize_text_field( $atts['channel'] );
		$count   = absint( $atts['count'] );
		$columns = sanitize_text_field( $atts['columns'] );

		if ( empty( $channel ) ) {
			return '<p class="pt-no-videos">' . esc_html__( 'Please specify a channel using the channel attribute.', 'peertube-video-manager' ) . '</p>';
		}

		$api    = new PT_API();
		$result = $api->get_channel_videos( $channel, array(
			'count' => $count,
			'start' => 0,
			'sort'  => '-publishedAt',
		) );

		if ( empty( $result['data'] ) ) {
			return '<p class="pt-no-videos">' . sprintf(
				/* translators: %s: channel name */
				esc_html__( 'No videos found in channel "%s".', 'peertube-video-manager' ),
				esc_html( $channel )
			) . '</p>';
		}

		$base_url = get_option( 'pt_vm_base_url', 'https://video3.cappital.co' );

		ob_start();
		$grid_class = 'pt-video-grid';
		
		// Handle columns parameter
		if ( 'auto' !== $columns && is_numeric( $columns ) ) {
			$columns_num = absint( $columns );
			if ( $columns_num >= 1 && $columns_num <= 6 ) {
				$grid_class .= ' pt-video-grid-cols-' . $columns_num;
			}
		} elseif ( '1' === $columns || 'single' === strtolower( $columns ) ) {
			$grid_class .= ' pt-video-grid-cols-1';
		}

		echo '<div class="' . esc_attr( $grid_class ) . '">';

		foreach ( $result['data'] as $video ) {
			// Get full video data only if pluginData is missing
			if ( ! isset( $video['pluginData'] ) || empty( $video['pluginData'] ) ) {
				$full_video = $api->get_video( $video['uuid'] );
				if ( $full_video ) {
					$video = $full_video;
				}
			}
			self::render_video_card( $video, $base_url, $api );
		}

		echo '</div>';
		return ob_get_clean();
	}

	/**
	 * Render video card
	 *
	 * @param array  $video Video data.
	 * @param string $base_url Base URL.
	 * @param PT_API $api API instance.
	 * @since 1.0.0
	 */
	private static function render_video_card( $video, $base_url, $api ) {
		$template_path = PT_VM_PLUGIN_DIR . 'templates/video-card.php';
		
		if ( file_exists( $template_path ) ) {
			include $template_path;
		}
	}
}

